...
export const signup = async (req, res) => {
  try {
    ...
    res.cookie("jwt", createToken(email, user._id), {
      ...
      sameSite: isProduction ? "none" : "lax", // cookie can be sent in cross-site requests : none
...
export const login = async (req, res) => {
  try {
    ...
    const comparePassword = await compare(password, user.password); // compare(fe password, db password)
...
//     originalname: 'raj.png',
//     // console.log(fileName); // uploads/profiles/7867543467+raj.png ==> uploads/profiles/7867543467raj.png
...
export const addProfileImage = async (req, res) => {
  ...
  try {
    ...
    // Use it for cleanup tasks that should happen regardless of success or failure, such as:
    ...
    if (fileName) {
      try {
        unlinkSync(fileName);
        // So unlinkSync() does use process.cwd() automatically when you pass a relative path.

        // Absolute paths start with / ==> "/home/rj/photo.jpg" or drive letter ==> C:\
        // linux absolute path format : /home/rj/Documents/file.txt and window ==> D:\Movies\Avengers.mp4

        // "uploads/photo.jpg"  // Relative
        // "./photo.jpg"        // Relative
...
////////// removeProfileImage (not cloudinary based) //////////////////////////////////////
